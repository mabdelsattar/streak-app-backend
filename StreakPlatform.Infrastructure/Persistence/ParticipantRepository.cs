...
public class ParticipantRepository : IParticipantRepository
{
    ...
    public Task<bool> IsActiveAsync(Guid userId, Guid streakId, CancellationToken ct = default) =>
        _db.Participants.AnyAsync(p => p.UserId == userId && p.StreakId == streakId && p.IsActive, ct);

    public Task<Participant?> GetAsync(Guid userId, Guid streakId, CancellationToken ct = default) =>
        _db.Participants.FirstOrDefaultAsync(p => p.UserId == userId && p.StreakId == streakId, ct);
